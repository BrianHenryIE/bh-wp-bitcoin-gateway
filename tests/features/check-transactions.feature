Feature: Check Bitcoin transactions for addresses or orders
  ...
  Scenario: Check help command is available
    When I run `wp help bh_bitcoin check_transactions`
  ...
  Scenario: Check transactions with non-existent order ID
    When I try `wp bh_bitcoin check_transactions 99999`
    Then the return code should not be 0

  Scenario: Check transactions with invalid Bitcoin address
    When I try `wp bh_bitcoin check_transactions invalid-address`
    Then the return code should not be 0
  ...
  Scenario: Check transactions with table format (default)
    ...
    When I try `wp bh_bitcoin check_transactions {ORDER_ID}`
    Then the return code should not be 0
  ...
  Scenario: Check transactions with JSON format
    ...
    When I try `wp bh_bitcoin check_transactions {ORDER_ID} --format=json`
    Then the return code should not be 0
  ...
  Scenario: Check transactions with CSV format
    ...
    When I try `wp bh_bitcoin check_transactions {ORDER_ID} --format=csv`
    Then the return code should not be 0
  ...
  Scenario: Check transactions with YAML format
    ...
    When I try `wp bh_bitcoin check_transactions {ORDER_ID} --format=yaml`
    Then the return code should not be 0
  ...
  Scenario: Check transactions with debug flag
    ...
    When I try `wp bh_bitcoin check_transactions {ORDER_ID} --debug=bh-wp-bitcoin-gateway`
    Then the return code should not be 0

  Scenario: Check transactions with valid Bitcoin address format
    When I try `wp bh_bitcoin check_transactions bc1qar0srrr7xfkvy5l643lydnw9re59gtzzwf5mdq`
    Then the return code should not be 0
